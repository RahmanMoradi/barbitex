...
class OrderController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return $this->response(1, $data, $meta, __('list orders'));
    ...
    public function show($id)
    {
        ...
        return $this->response(1, [new OrderResource($order)], null, __('order information'));
    }
    ...
    public function store(Request $request)
    {
        try {
            return \DB::transaction(function () use ($request) {
                ...
                return $this->response(1, [new OrderResource($order)], null, __('your order has been successfully registered'));
            });
        } catch (\Exception $exception) {
            return $this->response(0, [], null, __('the operation failed'));
        }
    }
    ...
    {
        $typeTxt = $request->type === 'buy' ? __('buy') : __('sell');
        Wallet::create(
            [
                'user_id' => Auth::guard('api')->id(),
                ...
                'description' => __('withdrawal for order', ['type' => $typeTxt]),
                'type' => 'decrement',
                'status' => 'done'
            ]
        );
    ...
    private function incrementBalance(Request $request, $order, $currency)
    {
        Wallet::create(
            [
                ...
                'wallet' => 'global_wallet',
                'description' => __('deposit for order number', ['orderId' => $order->id]),
                'type' => 'increment',
