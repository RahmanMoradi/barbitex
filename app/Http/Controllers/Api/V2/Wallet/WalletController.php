            ...
            if ($request->has('filter') && $request->get('filter') == 'count') {
                if ($count && Helper::numberFormatPrecision($count->balance, $currency->decimal) > 0) {
                    $balances->push([
                        ...
                        'markets' => MarketResource::collection(Market::where('currency_buy', $currency->id)->where('status', 1)->get())
                    ]);
                }
            } else {
                $balances->push([
...
//            Notification::send( User::whereRole( 2 )->get() , new SendNotificationToAdmin( 'orderCreate' , $order->id ) );
    ...
    public function withdrawalStore(Request $request, $currency)
    {
        if ($request->has('internal') && $request->internal === '1') {
            return $this->internal($request, $currency);
        } else {
            return $this->external($request, $currency);
        }
    }
...
//            }
    ...
    public function transfer(Request $request)
    {
        ...
        $balance = Balance::where('currency', $currency->symbol)
            ->where('user_id', Auth::id())->first();
        if ($request->amount > $balance->balance_free) {
            return $this->response(0, [], [], __('balance insufficient'));
        }
        ...
        try {
            \DB::transaction(function () use ($request, $currency) {
                Wallet::create([
                    ...
                    'description' => __('global transfer to', ['user_id' => $request->user_id]),
                ]);
                Balance::createUnique($currency->symbol, Auth::user(), -$request->amount);
                ...
                if (Settings::get('autoWithdraw') == '1') {
                    Balance::createUnique($currency->symbol, User::find($request->user_id), $request->amount);
                }
                return $this->response(1, [], [], __('operation completed successfully'));
            });
        } catch (\Exception $exception) {
    ...
    private function internal(Request $request, $currency)
    {
        $validate = Validator::make($request->all(), [
            'wallet' => 'required|numeric|exists:users,id',
            'qty' => 'required|numeric'
        ]);
        if ($validate->fails()) {
            return $this->validateResponseFail($validate);
        }
        $balance = Balance::where('user_id', Auth::guard('api')->id())->where('currency', $currency)->first();
        $balance = $balance ? $balance->balance : 0;
        if ($request->qty > $balance) {
            return $this->response(0, [], [], __('balance insufficient'));
        }
        try {
            \DB::transaction(function () use ($request, $currency) {
                Wallet::create([
                    ...
                    'status' => 'done',
                    'wallet' => $request->wallet,
                    'description' => __('global transfer to', ['user_id' => $request->wallet]),
                ]);
                Balance::createUnique($request->currency, Auth::user(), -$request->qty);

                $wallet = Wallet::create([
                    'user_id' => $request->wallet,
                    'currency' => $currency,
                    'price' => $request->qty,
                    'type' => 'increment',
                    'status' => 'done',
                    'wallet' => Auth::guard('api')->id(),
                    'description' => __('global transfer from', ['user_id' => Auth::guard('api')->id()]),
                ]);

                Balance::createUnique($currency, User::find($request->wallet), $request->qty);
    ...
    private function external(Request $request, $currency)
    {
        if ($currency === 'IRT') {
            $validate = Validator::make($request->all(), [
