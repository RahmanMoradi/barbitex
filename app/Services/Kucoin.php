...
use App\Models\Webazin\Kucoin\Facades\Kucoin as Api;
...
class Kucoin implements Withdrawal
{
    ...
    public static function withdraw(Wallet $wallet)
    {
        ...
        try {
            $transfer = Api::account()->innerTransferV2(
                uniqid('', true), $wallet->currencyRelation->symbol, 'trade', 'main', $qty
            );
        ...
        try {
            ...
            $waithdraw = Api::withdrawal()->apply($params);
            if (isset($waithdraw['withdrawalId'])) {
                $wallet->update([
                    'status' => 'done',
                    'service_id' => $waithdraw['withdrawalId']
                ]);
    ...
    public static function checkDeposit($currency, $txidRequest)
    {
        ...
        foreach ($deposits as $deposit) {
            ...
            $has = $deposit['walletTxId'] == $txidRequest;
            if ($has) {
    ...
    public function createOrder($symbol, $count, $price, $type)
    {
        ...
        try {
            $params = [
                ...
                'symbol' => $symbol,
                'type' => 'limit',
                'size' => Helper::numberFormatPrecision($count, $currency->decimal_size),
                'price' => Helper::numberFormatPrecision($price, $currency->decimal_size),
            ];
            BalanceUpdateEvent::dispatch(Auth::check() ? Auth::id() : Auth::guard('api')->id(), Market::whereSymbol($symbol)->first(), null);
            return Api::order()->create($params);
        } catch (\Exception $exception) {
            foreach (Admin::all() as $admin) {
                SendNotification::sendMessage($admin, $exception->getMessage());
            }
    ...
    public function createNetworkList(Currency $currency)
    {
        ...
        try {
            $response = Api::deposit()->getAddresses($currency->symbol);
            if (count($response) > 0) {
                foreach ($response as $network) {
                    try {
                        if (count($response) == 1) {
                            $list = collect(Api::currency()->getList());
                            $detail = $list->where('currency', $currency->symbol)->first();
                        } else {
                            $detail = Api::currency()->getDetail($currency->symbol, strtolower($network['chain']));
                        }

                        $default = Api::deposit()->getAddress($currency->symbol);
                        $data = [
                            ...
                            'withdrawMax' => '',
                            'minConfirm' => isset($network['confirms']) ? $network['confirms'] : '',
                            'unLockConfirm' => '0',
                            'isDefault' => count($response) == 1 || $network['chain'] == $default['chain'],
                        ];
                        Network::create($data + [
                ...
                Api::deposit()->createAddress($currency->symbol);
                self::createNetworkList($currency);
            }
