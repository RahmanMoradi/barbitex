...
{{--                <th>کل(تومان)</th>--}}
            ...
            @foreach ($seller as $key=> $item)
                <tr class="cursor-pointer" style="background-color: none;background-image: linear-gradient(90deg,rgba(182,54,71,0.3) {{$item[1]}}%, #fff0 0%, rgb(255 255 255 / 0%) 0%)" wire:click.prevent="$emitTo('market.order', 'setAmountFromBook',{{$key}},{{$item[0]}},'buy')">
                    <td class="p-0"  style="text-align: right">{{ $item[0] }}</td>
                    <td class="p-0 text-danger" style="text-align: left">{{ $key }}</td>
